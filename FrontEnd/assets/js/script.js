...
async function filtersCategories() {
    ...
    buttons.forEach((button) => {
        button.addEventListener("click", (btn) => {
            const buttonId = btn.target.id; // Cible l'id du bouton au click
...
async function addCategoriesModal() {
    const select = document.querySelector("form select"); // Récupère l'élément "select"
    const categoriesModal = await getCategories(); // Récupération des catégories

    categoriesModal.forEach(category => {
        const option = document.createElement("option"); // Crée un élément "option" pour chaque catégorie
        option.value = category.id; // Chaque option obtient l'id d'une catégorie donnée
        option.textContent = category.name; // Chaque option est nommée par le nom d'une catégorie donnée
        select.appendChild(option); // Chaque "option" est placée dans "select"
    });
};
